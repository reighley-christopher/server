...
function update(name){
   fs.readFile("/nfs/export/UTIL/components/"+name, {encoding:"ASCII"} ,function(err, data){
...
function update_glyphs(){
   glyphs_text = "";
   fs.readdir("/nfs/export/UTIL/glyphs/", function(err, files){
     for(f in files) {
        fs.readFile("/nfs/export/UTIL/glyphs/"+files[f], {encoding:"ASCII"}, function(err, data){
...
fs.readdir("/nfs/export/UTIL/components/", function(err, files){
  for(f in files){ update(files[f]) };
  });

update_glyphs();

watcher = fs.watch("/nfs/export/UTIL/components", function(event, name){ update(name);});
watcher = fs.watch("/nfs/export/UTIL/glyphs", function(event, name){ update_glyphs() });
...
app.all("/start/:name", function(req, res){
  ...
  var record = {};
  var sp = "/nfs/export/UTIL/bin/run_slave";
